...
export async function verifyEncryptedResource(base64Payload: string, passphrase: string): Promise<boolean> {
  try {
    // Clean base64
    base64Payload = base64Payload.replace(/\s+/g, '');
    const raw = Uint8Array.from(atob(base64Payload), c => c.charCodeAt(0));
    ...
    // If it decrypts without throwing, the token is valid
    return true;
  } catch (err) {
    // Invalid token or corrupted file
